...
function obj!(all_vars::Vector)
    ...
    μ_1, σ_1, μ_2, σ_2, μ_3, σ_3, μ_4, σ_4, μ_5, σ_5,
    μ_6, σ_6, μ_7, σ_7,t1, t2, t3, t4, t5, t6, t7= all_vars
...
bdd_lower = [i[1] for i in bdd]; bdd_lower = repeat(bdd_lower,toInt((tot_p-7)/2))
bdd_upper = [i[2] for i in bdd]; bdd_upper = repeat(bdd_upper,toInt((tot_p-7)/2))
append!(bdd_lower,[0.001 0.001 0.001 0.001 0.001 0.001 0.001])
append!(bdd_upper,[0.999 0.999 0.999 0.999 0.999 0.999 0.999])
function random_start()
    # temp = [rand(Uniform(bdd_lower[i],bdd_upper[i])) for i in 1:tot_p-7]|>Array
    temp = []
    append!(temp, repeat([1/7],7))
    # append!(temp, [1/14, 2/14, 2.5/14, 3/14, 2.5/14, 2/14, 1/14])
end
function myconstraint1(x::Vector, grad::Vector)
    return(sum(x[tot_p-7:tot_p])-1)
end
