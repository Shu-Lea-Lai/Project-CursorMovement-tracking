...
for iblock = 1:numBlock
    ...
    for itrial = 1:numTrialpB
        ...
        theta_sol = ode45(@(t,y) (b - a*sin(y)), tspan, x0);
        ...
        while 1
            ...
            [x, theCursors, theDots] = checktime(theCursors,theDots, x,time0, 2);
            if (GetSecs > nextFlip)
                
                [x, theCursors, theDots] = checktime(theCursors,theDots, x,time0,3);
                %% movment of x
                innerloop;
                ...
                [x, theCursors, theDots] = checktime(theCursors,theDots, x,time0,4);
                ...
                [x, theCursors, theDots] = checktime(theCursors,theDots, x,time0,5);
                
                textureflip([xdot_i-1,Y_pos_dot-1 ,xdot_i+1 ,Y_pos_dot+1 ], pic_button);
                Screen('DrawDots', win, [xdot_i, Y_pos_dot], dotsize, dotcolor);%, center, 1)
                Screen('Close',pic_button);
                Screen('Close',pic_bar);
                [x, theCursors, theDots] = checktime(theCursors,theDots, x,time0,5.5);
                Screen('Flip', whichScreen);
                [x, theCursors, theDots] = checktime(theCursors,theDots, x,time0,6);
                ...
                [x, theCursors, theDots] = checktime(theCursors,theDots, x,time0,7);
            end
            [x, theCursors, theDots] = checktime(theCursors,theDots, x,time0,8);
            loop = loop + 1;
        end
...
function [x, theCursors, theDots] = checktime(theCursors,theDots, x, time0,point)
    between = GetSecs - time0;
    if between*1000>10
%         m=[point,between*1000];
        [MouseX, MouseY, ~] = GetMouse(win);
        theCursors = [theCursors ; MouseX MouseY]; 
        theDots = [theDots ; xdot_i Y_pos_dot]; 
        x = GetSecs;
