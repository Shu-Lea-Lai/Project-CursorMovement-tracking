...
try
    ...
    if (debug==1|debug==2)
        ...
        [win,rect] = Screen('OpenWindow',0,[175 175 175],[100 100 900 400]);%max(Screen('Screens'));
    else %if i'm testing
...
%     Screen('DrawText',win,rest_index,50,50,50); 
%     Screen('Flip',win);
%     
%     legal=0;
%     if debug ~= 0
...
%     end 
    ...
    Screen('OpenWindow',win ,[175 175 175],[100 100 900 400]);%max(Screen('Screens'));
    ...
    Screen('DrawText',win, 'Drag mouse (i.e. hold button down) to draw',50,50,50 );
    Screen('Flip', win);
    while (1)
    ...
    Screen('DrawText',win,'Drag mouse (i.e. hold button down) to draw',50,50,0);

    % Loop and track the mouse, drawing the contour
    [theX,theY] = GetMouse(win);
    thePoints = [theX theY];
    Screen('DrawLine',win,  255,theX,theY,theX,theY);
    Screen('Flip', win);
    sampleTime = 0.01;
    startTime = GetSecs;
